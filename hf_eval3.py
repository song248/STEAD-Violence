...
PRED_DIR = f"{BASE_DIR}/predict-4sec-8fps-1"
...
def select_best_threshold(y_true: np.ndarray, prob: np.ndarray) -> Tuple[float, Dict[str, float]]:
    ...
    - 퇴화 케이스:
      * GT가 전부 0: t = nextafter(max(prob), +inf)  → 모두 0 예측( FP 최소 )
      * GT가 전부 1: t = nextafter(min(prob), -inf) → 모두 1 예측( FN 최소 )

    - 일반 케이스 타이브레이커:
      1) F1 동률 → |pred_pos - true_pos|가 작은 t
      2) 여전히 동률 → 더 큰 t (불필요한 FP 억제)
    """
    n = len(y_true)
    n_pos = int(y_true.sum())
    n_neg = n - n_pos

    # 안전: prob에 NaN만 있는 경우 방지
    ...
    # ----- 퇴화 케이스 처리 -----
    if n_pos == 0:
        # 모든 GT가 0 → FP 최소화: max(prob)보다 큰 값으로 컷
        t = float(np.nextafter(np.nanmax(prob), np.inf)) if len(prob) else 1.0
        # 성능 지표(참고용)
        y_pred = (prob >= t).astype(int)
        tp, fp, tn, fn = confusion_counts(y_true, y_pred)
        return t, precision_recall_f1_acc(tp, fp, tn, fn)

    if n_neg == 0:
        # 모든 GT가 1 → FN 최소화: min(prob)보다 작은 값으로 컷
        t = float(np.nextafter(np.nanmin(prob), -np.inf)) if len(prob) else 0.0
        y_pred = (prob >= t).astype(int)
        tp, fp, tn, fn = confusion_counts(y_true, y_pred)
        return t, precision_recall_f1_acc(tp, fp, tn, fn)
    ...
    best = {
        ...
        "recall": -1.0,
        "accuracy": -1.0,
        "gap": float("inf"),  # |pred_pos - true_pos|
    }
    ...
    for t in candidates:
        ...
        f1 = metrics["f1"]
        gap = abs(int(y_pred.sum()) - true_pos)
        ...
        if f1 > best["f1"]:
            ...
            if gap < best["gap"]:
                choose = True
            elif gap == best["gap"] and t > best["t"]:
                choose = True
